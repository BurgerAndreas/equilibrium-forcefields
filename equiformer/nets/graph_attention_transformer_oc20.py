...
class GraphAttentionTransformerOC20(torch.nn.Module):
    ...
    def __init__(
        ...
        if self.out_drop != 0.0:
            self.out_dropout = EquivariantScalarsDropout(
        ...
        if self.use_auxiliary_task and not self.use_attention_head:
            ...
            self.auxiliary_head = GraphAttention(
                self.irreps_feature,
                self.irreps_node_attr,
                self.irreps_edge_attr,
                irreps_out_auxiliary,
                self.fc_neurons,
                self.irreps_head,
                self.num_heads,
                self.irreps_pre_attn,
                self.rescale_degree,
                self.nonlinear_message,
    ...
    def forward(self, data):
        ...
        # auxiliary IS2RS
        if self.use_auxiliary_task:
            outputs_aux = self.auxiliary_head(
                node_input=node_features,
                node_attr=node_attr,
                edge_src=edge_src,
                edge_dst=edge_dst,

...
if __name__ == "__main__":
    ...
    torch.manual_seed(0)
    tp_rs = TensorProductRescale(
        irreps_1, irreps_2, irreps_mid, instructions, bias=False, rescale=False
    )
    ...
    print(
        "[TP] before rescaling difference: {}".format(
            torch.max(torch.abs(out_tp - out_tp_rs))
        )
    )
    ...
    out_tp_rs = tp_rs.forward(inputs_1, inputs_2)
    print(
        "[TP] after rescaling difference: {}".format(
            torch.max(torch.abs(out_tp - out_tp_rs))
        )
    )
    ...
    torch.manual_seed(0)
    fctp_rs = FullyConnectedTensorProductRescale(
        irreps_1, irreps_2, irreps_out, bias=False, rescale=False
    )

    out_fctp = fctp.forward(inputs_1, inputs_2)
    out_fctp_rs = fctp_rs.forward(inputs_1, inputs_2)
    print(
        "[FCTP] before rescaling difference: {}".format(
            torch.max(torch.abs(out_fctp - out_fctp_rs))
        )
    )
    ...
    out_fctp_rs = fctp_rs.forward(inputs_1, inputs_2)
    print(
        "[FCTP] after rescaling difference: {}".format(
            torch.max(torch.abs(out_fctp - out_fctp_rs))
        )
    )

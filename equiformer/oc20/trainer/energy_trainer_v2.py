...
class EnergyTrainerV2(BaseTrainerV2):
    ...
    def predict(self, loader, per_image=True, results_file=None, disable_tqdm=False):
        ...
        if self.config["task"].get("write_pos", False):
            torch.save(
                pos_preds,
                os.path.join(
                    self.run_dir, "pos_pred_{}.pt".format(distutils.get_rank())
                ),
            )
        ...
        for epoch_int in range(start_epoch, self.config["optim"]["max_epochs"]):
            ...
            for i in range(skip_steps, len(self.train_loader)):
                ...
                if self.use_interpolate_init_relaxed_pos:
                    batch = [
                        interpolate_init_relaxed_pos(batch_data) for batch_data in batch
                    ]
                ...
                if self.step % eval_every == 0 or i == (len(self.train_loader) - 1):
                    ...
                    if self.val_loader is not None:
                        val_metrics = self.validate(
                            split="val", disable_tqdm=disable_eval_tqdm, use_ema=False
                        )
                        if (
    ...
    def _compute_loss(self, out, batch_list):
        ...
        if self.use_auxiliary_task:
            ...
            loss_aux = self.loss_fn["force"](
                _mask_input(out["pos"], tag_mask), _mask_input(delta_pos, tag_mask)
            )
    ...
    def _compute_auxiliary_task_weight(self):
        ...
        weight = weight - weight_range * min(
            1.0, ((self.step + 0.0) / self.total_steps)
        )

...
# run with `source multiruns/md22_equ.sh`
...
for mol in ${md22[@]}; do
    # launchrun +use=deq +cfg=fpc_of wandb_tags=["md22"] target=$mol
    launchrun model.num_layers=1 +cfg=dd wandb_tags=["md22"] dname=md22 target=$mol
    launchrun model.num_layers=4 +cfg=dd  wandb_tags=["md22"] dname=md22 target=$mol
    launchrun model.num_layers=8 +cfg=dd  wandb_tags=["md22"] dname=md22 target=$mol
done

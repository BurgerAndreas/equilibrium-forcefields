...
def _init_deq(
    ...
    **kwargs,
    ...
    # Using ln_type='none' in `kwargs` can also skip it.
    if torchdeq_norm.ln_type in [None, "none", False]:
        print(f"Not applying torchdeq normalization.")
        pass
    elif "both" in torchdeq_norm.ln_type:
        print(
            f"Applying both weight and spectral normalization with kwargs: {norm_kwargs}"
        )
        norm_kwargs = copy.deepcopy(torchdeq_norm)
        norm_kwargs.pop("ln_type")
        apply_norm(self.blocks, ln_type="weight_norm", **norm_kwargs)
        apply_norm(self.blocks, ln_type="spectral_norm" ** norm_kwargs)
    else:
        print(
            f"Applying {torchdeq_norm.ln_type} normalization with kwargs: {torchdeq_norm}"
        )

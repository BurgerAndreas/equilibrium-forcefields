...
def hydra_wrapper(args: DictConfig) -> None:
                ...
                if max_repeats > 1 and rep == max_repeats - 1:
                    for reps, _name in zip([reps_fp1, reps_fp2, reps_fp2_wreuse], ["fp1", "fp2", "fp2_wreuse"]):
                        if len(reps) > 1:
                            ...
                            pairwise_distances = get_pairwise_distances(reps) # TODO: replace this with a similarity matrix
                            ...
                            if _max > repeat_tol:
                                ...
                                print(f"\n{cnt}: {max_repeats} repeats avg distance between {len(reps)} fixed-points ({_name}):", _mean)
                                # print the max distance
                                print(f"{cnt}: {max_repeats} repeats max distance between {len(reps)} fixed-points ({_name}):", _max)
    ...
    if max_repeats > 1:
        ...
        print("Average distance between repeated fixed-points, averaged over data points:", torch.mean(fp_repeat_avg_distances).item())
        print("Max     distance between repeated fixed-points, averaged over data points:", torch.mean(fp_repeat_max_distances).item())
    ...
    print("Avg |fp2 - fp1|        =", torch.mean(d_fp2_fp1).item(), "<- should be larger")
    print("Avg |fp2_wreuse - fp1| =", torch.mean(d_fp2wreuse_fp1).item(), "<- should be larger")
    _rel_err = torch.mean(
        d_fp2_wo_reuse / d_fp2_fp1
    ).item()
    print("Avg |fp2 - fp2_wreuse| / |fp2 - fp1| =", _rel_err, "<- should be small")
    print("Avg |fp2 - fp2_wreuse| / |fp2| =", torch.mean(d_fp2_wo_reuse_rel).item(), "<- should be small")

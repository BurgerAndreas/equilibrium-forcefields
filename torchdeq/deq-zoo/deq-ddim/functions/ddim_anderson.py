...
def compute_multi_step(
    xt, all_xT, model, et_coeff, et_prevsum_coeff, T, t, image_dim, xT, **kwargs
):
    with torch.no_grad():
        ...
        if len(all_seqs) > 0:
            for idx in all_seqs:
                ...
                et_prevsum[idx + 1 : idx + 1 + T] -= torch.repeat_interleave(
                    prev_cumsum, T, dim=0
                )
...
@torch.no_grad()
def anderson(
    f, x0, X, F, H, y, args, m=5, lam=1e-3, max_iter=15, tol=1e-2, beta=1.0, logger=None
):
    ...
    with torch.no_grad():
        ...
        for k in range(2, max_iter):
            ...
            H[:, 1 : n_ + 1, 1 : n_ + 1] = (
                ...
                + lam * torch.eye(n_, dtype=x0.dtype, device=x0.device)[None]
            )
            t4 = time.time()
            alpha = torch.solve(y[:, : n_ + 1], H[:, : n_ + 1, : n_ + 1])[0][
                :, 1 : n_ + 1, 0
            ]  # (bsz x n)
            t5 = time.time()
            X[:, k % m] = (
                beta * (alpha[:, None] @ F[:, :n_])[:, 0]
                + (1 - beta) * (alpha[:, None] @ X[:, :n_])[:, 0]
            )
            F[:, k % m], log_metrics = f(xt=X[:, k % m].view(x0.shape), **args)
            t6 = time.time()

            residual = (F[:, k % m] - X[:, k % m]).norm().item()
            normalized_residual = (F[:, k % m] - X[:, k % m]).norm().item() / (
                1e-5 + F[:, k % m].norm()
            ).item()
            ...
            if logger is not None:
                ...
                log_metrics["samples"] = [
                    wandb.Image(X[:, k % m].view_as(x0).to("cpu")[ts])
                    for ts in args["plot_timesteps"]
                ]
                log_metrics["setup"] = time_logger["setup"]
                log_metrics["total_time_per_iter"] = (
                    time_logger["total_time_per_iter"] / iter_count
                )
                ...
                for key, val in time_logger.items():
                    if key not in log_metrics:
                        ...
                        log_metrics[key] = val
                log_metrics["perc_time_forward_call"] = (
                    time_logger["forward call-unet"]
                    * 100
                    / time_logger["total_time_per_iter"]
                )
                logger(log_metrics)
...
def fp_implicit_iters_anderson(
    x,
    model,
    b,
    args=None,

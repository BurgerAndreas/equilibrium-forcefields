...
def flow_loss(flow_preds, flow_gt, valid, gamma=0.8, max_flow=MAX_FLOW, cal_epe=True):
...
class Logger:
    def __init__(self, scheduler):
        ...
        self.writer.global_steps = self.total_steps + 1
    ...
    def push(self, metrics):
        ...
        if (self.total_steps + 1) % SUM_FREQ == 0:
...
def train(args):
    ...
    for i in range(args.start_run, args.total_run + 1):
        ...
        log_training_stats(args, stats)

        # reset resume iters
        args.resume_iter = -1


def log_training_stats(args, stats):
...
def write_backward_stats(writer, info):
...
def train_once(args):
    ...
    while should_keep_training:
        ...
        for i_batch, data_blob in enumerate(train_loader):
            ...
            if (total_steps + 1) % SUM_FREQ == 0:
                writer_f = partial(write_backward_stats, writer=logger.writer)
            else:
                writer_f = None
            ...
            loss, metrics = model(image1, image2, flow, valid, fc_loss, writer=writer_f)
            ...
            if (total_steps + 1) % TIME_FREQ == 0:
                print(
                    f"Exp {args.name_per_run} Average Time: {(timer / TIME_FREQ):.3f}"
                )
                timer = 0
...
# run via:
# cd torchdeq/deq-zoo/deq-flow/
# pip install opencv-python einpos
# python main.py --stage kitti

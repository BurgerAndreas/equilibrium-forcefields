...
class FourierInjection(nn.Module):
    def __init__(self, in_channels, interm_channels, scale=1.0, n_layers=1):
        super().__init__()
...
class GaborInjection(nn.Module):
    def __init__(
        self, in_channels, interm_channels, scale=1.0, alpha=5.0, beta=1.0, n_layers=1
    ):
        super().__init__()
...
class GaussianFFNEmbedder(nn.Module):
    def __init__(self, in_channels, embedding_size=256, scale=10.0):
        super().__init__()

        self.bval = nn.Parameter(
            torch.randn(2, embedding_size) * 10, requires_grad=False
        )
    ...
    def forward(self, x):
        return torch.cat(
            [
                self.aval * torch.sin((2 * np.pi * x) @ self.bval),
                self.aval * torch.cos((2 * np.pi * x) @ self.bval),
            ],
            dim=1,
        )
...
class FFNInjection(nn.Module):
    def __init__(self, in_channels, interm_channels, scale=10.0):
        super().__init__()

        interm_channels = interm_channels * n_layers
        self.n_layers = n_layers

        self.embedder = GaussianFFNEmbedder(
            in_channels, interm_channels // 2, scale=scale
        )
...
def get_injection(
    ...
    **filter_options
    ...
    if filter_type == "fourier":
        return FourierInjection(
            in_channels, interm_channels, scale / layer_scale, n_layers=n_layers
        )
    elif filter_type == "gabor":
        if "alpha" in filter_options:
            ...
            alpha = 6.0
        return GaborInjection(
            in_channels, interm_channels, scale / layer_scale, alpha, n_layers=n_layers
        )
    elif filter_type == "siren_like":
        return SIRENInjection(
            in_channels, interm_channels, scale / layer_scale, n_layers=n_layers
        )
    elif filter_type == "relu":
        return FFNInjection(in_channels, interm_channels, 15.0, n_layers=n_layers)
    else:

...
class ConfusionMatrix:
    ...
    def update(self, pred, label):
        assert len(pred.shape) == len(label.shape) and [
            pred.shape[i] == label.shape[i] for i in range(len(pred.shape))
        ]
        pos_mask = pred == 1
        neg_mask = pred == 0
    ...
    def get_acc(self):
        return (self.stats["TP"] + self.stats["TN"]) / (
            self.stats["TP"] + self.stats["TN"] + self.stats["FP"] + self.stats["FN"]
        )
    ...
    def get_iou(self):
        return (self.stats["TP"]) / (
            self.stats["TP"] + self.stats["FN"] + self.stats["FP"]
        )
...
def batched_apply(
    f, inputs, batch_size=1024, f_kwargs={}, print_progress=False, device=None
):
...
def create_mesh(
    ...
    while head < num_samples:
        ...
        samples[head : min(head + max_batch, num_samples), 3] = (
            model(sample_subset)["output"].squeeze().detach().cpu()  # .squeeze(1)
        )
        head += max_batch

...
def train(
    ...
    for i in data_iter:
        ...
        model_outputs = model(
            train_batched_input[batch_indices],
            z_init[batch_indices],
            skip_solver=not args.no_skip_solver and i > 0,
        ...
        if do_log or do_visualize:
            ...
            with torch.no_grad():
                ...
                train_rec = postprocess(
                    batch2hw(model(train_batched_input)["output"], train_h, train_w)
                )
                ...
                test_rec = postprocess(
                    batch2hw(model(test_batched_input)["output"], test_h, test_w)
                )
            model.train()
            ...
            if do_log:
                ...
                data_iter.set_postfix(
                    {"Train PSNR": train_psnr, "Test PSNR": test_psnr}
                )
            ...
            if do_visualize:
                ...
                if vis_tag is None:
                    ...
                    fig.savefig(
                        "{:s}/step_{:d}.jpeg".format(args.vis_dir, i + 1), dpi=600
                    )
    return {
        "loss": loss_list,
...
def main(args):
    ...
    y_train, y_test = torch.tensor(y_train, dtype=torch.float32).to(
        args.device
    ...
    new_dict = get_summary_dict(
        args, 2, data_channels, args.filters, args.network_specs, args.input_scale
    )
    ...
    log_writer = open(
        "{:s}/logs.txt".format(args.log_dir), "w+" if not args.continue_run else "a+"
    )
    ...
    for model_id in summary_dict:
        ...
        train_psnr_mean, train_psnr_std = np.mean(train_psnr_final), np.std(
            train_psnr_final
        )
        test_psnr_mean, test_psnr_std = np.mean(test_psnr_final), np.std(
            test_psnr_final
        )
        avg_forward_step_mean, avg_forward_step_std = np.mean(
            model_dict["results"]["avg_forward_step"]
        ), np.std(model_dict["results"]["avg_forward_step"])

        log_writer.write(
            "Model: {:s} Dataset: {:s} Train mean: {:.4f}, std: {:.4f}; Test mean: {:.4f}, std: {:.4f}; Avg forward steps mean: {:.2f}, std: {:.2f}\n".format(
                model_id,
                args.dataset,
                train_psnr_mean,
                train_psnr_std,

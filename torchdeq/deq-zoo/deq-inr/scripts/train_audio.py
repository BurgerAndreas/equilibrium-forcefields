...
def train(
    ...
    z_inits = [
        torch.zeros(splits[i + 1] - splits[i], model.interm_channels).to(device)
    ...
    for i in data_iter:
        ...
        for j in range(len(splits) - 1):
            ...
            loss = criterion(
                model_outputs["output"].squeeze(-1),
                train_tgt[splits[j] : splits[j + 1]],
            )
        ...
        if do_log or do_vis:
            ...
            if do_log:
                ...
                log_writer.write(
                    f"Iter {i}, loss: {loss.item():.5e}, PSNR {test_psnr:.2f}\n"
                )
                log_writer.flush()
            ...
            if do_vis:
                ...
                scipy.io.wavfile.write(
                    "{:s}/test_step{:d}.wav".format(args.vis_dir, i),
                    args.audio_rate,
                    test_rec,
                )
                np.save(
                    "{:s}/test_step{:d}_source.npy".format(args.vis_dir, i), test_rec
                )
...
def test(args, test_data, model):
    ...
    with torch.no_grad():
        test_rec = postprocess(
            model(test_input.unsqueeze(-1).to(args.device), verbose=True)["output"]
        )
...
def main(args):
    ...
    t_arr = np.linspace(-1.0, 1.0, len(data), dtype=np.float32)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input_scale", default=25000.0, type=float)
    parser.add_argument(
        "--filter_type", default="fourier", choices=["fourier", "gabor", "siren_like"]
    )
    parser.add_argument("--gabor_alpha", default=3.0, type=float)

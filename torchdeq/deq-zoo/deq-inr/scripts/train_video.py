...
def train(
    ...
    indices_generator = batch_indices_generator(
        len(train_flattened_input), args.train_batch_size, shuffle=True
    )

    pix_cache = torch.zeros(train_h * train_w, 1024)

    # Main training Loop
    postfix = {"loss": np.inf, "psnr": 0.0, "forward_steps": 0}
    for i in data_iter:
        ...
        model_outputs = model(
            train_flattened_input[iter_indices].to(device),
            z_init,
            skip_solver=False,
            verbose=False,
        )
        loss, fc_loss = fp_correction(
            criterion,
            (model_outputs["output"], train_flattened_target[iter_indices].to(device)),
        ...
        if do_log or do_vis:
                ...
                test_rec = postprocess(
                    flat2hw(
                        batched_apply(
                            lambda x: model(
                                x.to(device),
                                torch.zeros(len(x), model.interm_channels).to(device),
                            ),
            ...
            if do_log:
                ...
                test_psnrs, test_psnr_mean, test_psnr_std = get_psnr_stats(
                    test_orig, test_rec
                )
                ...
                log_writer.write(
                    f"Iter {i}, loss: {loss.item():.5e}, PSNR mean: {test_psnr_mean:.3f}, PSNR std: {test_psnr_std:.3f}\n"
                )
                log_writer.flush()
            ...
            if do_vis:
                ...
                assert os.path.exists(
                    args.vis_dir
                ), "Visualization directory {:s} does not exist".format(args.vis_dir)
        ...
        test_rec = postprocess(
            flat2hw(
                batched_apply(
                    lambda x: model(
                        x.to(args.device),
                        torch.zeros(len(x), model.z_channels).to(args.device),
                    ),
                    (flattened_input,),
                    batch_size=args.test_batch_size,
                    print_progress=True,
...
def main(args):
    ...
    data = torch.tensor(
        skvideo.io.vread(video_path).astype(np.single) / 255.0, dtype=torch.float32
    )
    ...
    train_in = torch.tensor(
        np.stack(np.meshgrid(t_arr, h_arr, w_arr, indexing="ij"), axis=-1),
        dtype=torch.float32,
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_type", default="implicit", choices=["implicit", "siren", "ffn"]
    )
    ...
    parser.add_argument(
        "--filter_type", default="fourier", choices=["fourier", "gabor", "siren_like"]
    )
    parser.add_argument("--gabor_alpha", default=3.0)
    ...
    [
        ...
        for path in (args.log_dir, args.vis_dir, args.save_dir)
    ]

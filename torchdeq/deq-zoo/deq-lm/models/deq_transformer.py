    ...
    def __init__(
        ...
        **kwargs
        ...
        self.pos_ff = nn.ModuleList(
            [FFN(d_model, d_inner, dropout) for _ in range(n_layer)]
        )
...
class DEQTransformerLM(nn.Module):
    ...
    def forward(self, data, target, mems, **kwargs):
        ...
        if z_hist is not None and z_hist.nelement() > 0:
            assert (
                z_hist.shape[1] == u_hist.shape[1]
            ), "Padding fixed points and padding embedding dimensions don't agree"
        else:
            z_hist, u_hist = torch.zeros(bsz, 0, d_model), torch.zeros(
                bsz, 0, 3 * n_head * d_head
            )
        ...
        attn_mask += (
            torch.tril(torch.ones(q_len, k_len), diagonal=m_len - local_size) > 0
        )

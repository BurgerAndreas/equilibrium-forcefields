...
class PositionalEmbedding(nn.Module):
    ...
    def forward(self, pos_seq, bsz=None):
        ...
        pos_emb = torch.cat(
            [sinusoid_inp.sin(), sinusoid_inp.cos()], dim=0
...
class Attention(nn.Module):
    ...
    def forward(self, z, pos_emb, u, mems=None):
        ...
        w_head_q = w_head_q.view(
            bsz, n_head, d_head, qlen
        )  # bsz x n_head x d_head x qlen
        w_head_k = w_head_k.view(
            bsz, n_head, d_head, klen
        )  # bsz x n_head x d_head x klen
        ...
        attn_mask += (
            torch.tril(torch.ones(qlen, klen), diagonal=mlen - local_size) > 0
        )[None]
...
class DEQTransformerLM(nn.Module):
        ...
        if tie_projs:
            for i, tie_proj in enumerate(tie_projs):
                if tie_proj and div_val == 1 and d_model != d_embed:
                    self.crit.out_projs[i].weight.data = self.word_emb.emb_projs[
                        0
                    ].weight.data
    ...
    def forward(self, data, target, mems, **kwargs):
        ...
        if z_hist is not None and z_hist.nelement() > 0:
            assert (
                z_hist.shape[2] == u_hist.shape[2]
            ), "Padding fixed points and padding embedding dimensions don't agree"
        else:
            z_hist, u_hist = torch.zeros(bsz, d_model, 0), torch.zeros(
                bsz, 3 * d_model, 0
            )
        mlen = z_hist.shape[2]
        ...
        pos_emb = self.pos_drop(
            self.pos_emb(pos_seq)
        )  # bsz x d_model x (qlen + mlen) for positional embedding
        u_cat = torch.cat([u_hist, u_now], dim=2)

        z_now = torch.zeros(
            bsz, d_model, qlen, device=data.device
        )  # (B, D, L) for initial estimate of output
        ...
        z_out, info = self.deq(
            deq_func, z_now, sradius_mode=sradius_mode, backward_writer=writer
        )
        z_now = z_out[-1]

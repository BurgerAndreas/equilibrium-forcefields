...
parser.add_argument(
    "--eval_n_layer", type=int, default=12, help="number of total layers at evaluation"
)
parser.add_argument(
...
if args.resume_dir:
    ...
    model.apply(
        weights_init
    )  # Note: This applies weight_init recursively to modules in model
...
if args.multi_gpu:
    ...
    if (
        args.gpu0_bsz >= 0
        and args.batch_size != args.gpu0_bsz * torch.cuda.device_count()
    ):
        para_model = BalancedDataParallel(args.gpu0_bsz // args.batch_chunk, model).to(
            device
        )
    else:
...
def evaluate(eval_iter):
    ...
    if args.sradius_mode:
        print(
            "WARNING: You are evaluating with the power method at val. time. This may make things extremely slow."
        )
...
def train():
    ...
    for batch, (data, target, seq_len) in enumerate(train_iter):
        ...
        if (
            args.jac_loss_weight > 0
            and args.jac_loss_freq > 0
            and args.jac_incremental > 0
            and train_step % args.jac_incremental == 0
        ):
            logging(
                f"Adding 0.1 to jac. regularization weight after {train_step} steps"
            )

...
class ImplicitGraph(nn.Module):
    ...
    def __init__(
        self, args, in_features, out_features, num_node, kappa=0.99, b_direct=False
    ):
        ...
        self.n = num_node
        self.k = (
            kappa  # if set kappa=0, projection will be disabled at forward feeding.
        )
    ...
    def forward(self, X_0, A, U, phi, A_rho=1.0, fw_mitr=300, bw_mitr=300, A_orig=None):
        ...
        # Define variables
        if A_orig is None:
            W, A, B = self.W, A, b_Omega
        else:
            W, A, B = self.W, A_orig, b_Omega
...
register_norm_module(
    ImplicitGraph, "weight_norm", names=["W", "Omega_1", "Omega_2"], dims=[0, 0, 0]
)
register_norm_module(
    ImplicitGraph, "spectral_norm", names=["W", "Omega_1", "Omega_2"], dims=[0, 0, 0]
)

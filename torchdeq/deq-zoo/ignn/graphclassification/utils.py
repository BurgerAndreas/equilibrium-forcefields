...
def load_citation(
    dataset_str="cora", normalization="AugNormAdj", cuda=True, need_orig=False
):
    """
    ...
    return (
        [adj, adj_orig] if need_orig else adj,
        features,
        labels,
        idx_train,
        idx_val,
...
def load_citation_chain(normalization, cuda, need_orig=False):
    ...
    features = torch.FloatTensor(
        np.array(features.todense() if sp.issparse(features) else features)
    ).float()
    labels = torch.LongTensor(labels)
    labels = torch.max(labels, dim=1)[1]
    adj = sparse_mx_to_torch_sparse_tensor(adj).float()
...
def Evaluation(output, labels):
    ...
    #'''
    return metrics.f1_score(labels, binary_pred, average="micro"), metrics.f1_score(
        labels, binary_pred, average="macro"
    )
...
def projection_norm_inf(A, kappa=0.99, transpose=False):
    ...
    for idx in np.where(x > v)[0]:
        ...
        A_np[idx, :] = a
    A.data.copy_(
        torch.tensor(A_np.T if transpose else A_np, dtype=A.dtype, device=A.device)
    )
    return A
...
def load_txt_data(dataset_str="amazon-all", portion="0.06"):
    ...
    idx_train = list(
        np.loadtxt(
            "data/" + dataset_str + "/train_idx-" + str(portion) + ".txt", dtype=int
        )
    )
...
def sgc_precompute(features, adj, degree):
    ...
    return (
        torch.sparse.FloatTensor(
            features_index, features_value, torch.Size(features.shape)
        ),
        precompute_time,
    )

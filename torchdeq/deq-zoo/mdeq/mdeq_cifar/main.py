...
def parse_args():
    ...
    parser.add_argument(
        "--exp_id", help="model directory", type=str, default="cifar_mdeq"
    )
    parser.add_argument("--log_dir", help="log directory", type=str, default="")
    parser.add_argument("--data_dir", help="data directory", type=str, default="")
    parser.add_argument("--test_model", help="testModel", type=str, default="")
...
def train_model(args, logger, final_output_dir, tb_log_dir):
    ...
    if config.TRAIN.MODEL_FILE:
        ...
        logger.info(
            colored("=> loading model from {}".format(config.TRAIN.MODEL_FILE), "red")
        )
    ...
    if dataset_name == "imagenet":
        traindir = os.path.join(
            config.DATASET.ROOT + "/images", config.DATASET.TRAIN_SET
        )
...
def eval_model(args, logger, final_output_dir, tb_log_dir):
    ...
    if dataset_name == "imagenet":
        ...
        ), "Only CIFAR-10 and ImageNet are supported at this phase"
        classes = (
            "plane",
            "car",
            "bird",

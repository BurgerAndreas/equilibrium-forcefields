...
BN_MOMENTUM = 0.1
DEQ_EXPAND = (
    5  # Don't change the value here. The value is controlled by the yaml files.
)
NUM_GROUPS = (
    4  # Don't change the value here. The value is controlled by the yaml files.
)
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.bn3 = nn.BatchNorm2d(
            planes * self.expansion, momentum=BN_MOMENTUM, affine=False
        )
        self.relu = nn.ReLU(inplace=True)
        self.downsample = downsample
        self.stride = stride
...
class DownsampleModule(nn.Module):
    def __init__(self, num_channels, in_res, out_res):
        ...
        for k in range(level_diff):
            ...
            components = [
                ("conv", nn.Conv2d(inp_chan, intermediate_out, **kwargs)),
        ...
        self.net = nn.Sequential(
            OrderedDict(
                [
                    ...
                    (
                        "upsample",
                        nn.Upsample(scale_factor=2**level_diff, mode="nearest"),
                    ),
                ]
            )
        )
...
class MDEQModule(nn.Module):
    def __init__(self, num_branches, blocks, num_blocks, num_channels, fuse_method):
        ...
        self._check_branches(num_branches, blocks, num_blocks, num_channels)
                    ...
                    OrderedDict(
                        [
                            ...
                            (
                                "gnorm",
                                nn.GroupNorm(
                                    NUM_GROUPS // 2, num_channels[i], affine=True
                                ),
                            ),
                        ]
                    )
...
class MDEQ(nn.Module):
    def __init__(self, cfg, args, **kwargs):
        ...
        self.downsample = nn.Sequential(
            ...
            conv3x3(
                init_chansize,
                init_chansize,
                stride=(2 if self.downsample_times >= 2 else 1),
            ),
            nn.BatchNorm2d(init_chansize, momentum=BN_MOMENTUM),
            nn.ReLU(),
        )
        ...
        if self.downsample_times == 0 and self.num_branches <= 2:
            ...
            self.stage0 = nn.Sequential(
                nn.Conv2d(
                    self.init_chansize, self.init_chansize, kernel_size=1, bias=False
                ),
    ...
    def _make_stage(self, layer_config, num_channels):
        ...
        return MDEQModule(
            num_branches, block_type, num_blocks, num_channels, fuse_method
        )
    ...
    def _make_head(self, pre_stage_channels):
        ...
        for i, channels in enumerate(pre_stage_channels):
            incre_module = self._make_layer(
                head_block, channels, head_channels[i], blocks=1, stride=1
            )
        ...
        final_layer = nn.Sequential(
            nn.Conv2d(
                head_channels[len(pre_stage_channels) - 1] * head_block.expansion,
                self.final_chansize,
                kernel_size=1,
                stride=1,
                padding=0,
    ...
    def forward(self, x, train_step=-1, **kwargs):
        ...
        for i in range(1, self.num_branches):
            ...
            u_inj.append(
                torch.zeros(bsz, self.num_channels[i], H // 2, W // 2, device=x.device)

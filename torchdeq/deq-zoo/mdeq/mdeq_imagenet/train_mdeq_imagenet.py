...
parser.add_argument(
    "--num-classes", default=1000, type=int, metavar="N", help="Number of classes"
)
parser.add_argument(
...
parser.add_argument(
    "--lr", "--learning-rate", type=float, default=0.1, help="learning rate"
)

parser.add_argument(
    "--nesterov", action="store_true", help="Use Nesterov SGD optimizer"
)
parser.add_argument("--epochs", type=int, default=120, help="epochs")
parser.add_argument(
    "--wd", "--weight-decay", type=float, default=1e-4, help="weight decay"
)

parser.add_argument(
    "--imsize", default=256, type=int, metavar="N", help="image resize size"
)
parser.add_argument(
    "--imcrop", default=224, type=int, metavar="N", help="image crop size"
)
...
parser.add_argument(
    "--save-path", default="results/tmp", type=str, help="path to save results"
)
...
parser.add_argument(
    "--dali-cpu", action="store_true", help="Runs CPU based version of DALI pipeline."
)
    ...
    for i, data in enumerate(train_loader):
        ...
        if args.local_rank == 0 and scheduler.iter_counter == 630000:
            ...
            save_checkpoint(
                ...
                True,
                path=args.save_path,
                filename="pretrain-checkpoint.pth",
            )

        # Log training info
        if i % args.print_freq == 0 and args.local_rank == 0:
            tfboard_writer.add_scalar(
                "train/learning-rate", lr, epoch * train_loader_len + i
            )
            logger.info(
...
def validate(val_loader, model, train_step):
    ...
    for i, data in enumerate(val_loader):
        ...
        if args.local_rank == 0:
            ...
            if i % args.print_freq == 0:
                logger.info(
                    "Test: [{0}/{1}] Test Loss {loss.val:.3f} (avg={loss.avg:.3f}) "
                    "Acc1 {top1.val:.3f} (avg={top1.avg:.3f}) Acc5 {top5.val:.3f} (avg={top5.avg:.3f})".format(
    ...
    val_loader.reset()
    top1 = (
        torch.tensor([top1.avg]).cuda()
        if args.local_rank == 0
        else torch.tensor([0.0]).cuda()
    )
    top5 = (
        torch.tensor([top5.avg]).cuda()
        if args.local_rank == 0
        else torch.tensor([0.0]).cuda()
    )

...
def init_solver_info(bsz, device, init_loss=1e8):
    ...
    trace_dict = {
        ...
        # added
        "abs64": [torch.tensor(init_loss, device=device).repeat(bsz)],
        "rel64": [torch.tensor(init_loss, device=device).repeat(bsz)],
    }
    lowest_dict = {
        "abs": torch.tensor(init_loss, device=device).repeat(bsz),
        "rel": torch.tensor(init_loss, device=device).repeat(bsz),
    ...
    lowest_step_dict = {
        "abs": torch.tensor(0.0, device=device).repeat(bsz),
        "rel": torch.tensor(0.0, device=device).repeat(bsz),
    }
...
def update_state(
    ...
    for mode in ["rel", "abs"]:
        ...
        if mode == stop_mode:
            ...
            lowest_xest = lowest_xest.detach().clone()
...
def solver_stat_from_info(stop_mode, lowest_dict, trace_dict, lowest_step_dict):
    """
    ...
    """
    return SolverStat.from_solver_info(
